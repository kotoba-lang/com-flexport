(ns flexport.main
  "Kotodama WASM entrypoint for the Flexport clean-room actor (L4) — Clojure port.
  ...
  facts (`flexport.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flexport")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"flexport.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flexport-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flexport-compat/ready)
